...
module.exports = async function (hre: HardhatRuntimeEnvironment) {
    ...
    console.log(`OFTAdapter deployer: ${deployer}`)
    ...
    const token = tokenInfo.token ?? (await hre.deployments.get(`${tokenInfo.name}`)).address
    console.log(`Token address: ${token}`)

    await deploy(getDeployName(tokenInfo), {
        contract: 'OmniAdapterToken',
        from: deployer,
        args: [token, endpointAddr, deployer],
    ...
    console.log(`AdapterOFT deployed`)
}

module.exports.tags = ['AdapterOFT', 'test']
...
module.exports.skip = async ({ network }: HardhatRuntimeEnvironment) =>{
    ...
    return new Promise((resolve) => {
        resolve(tokenInfo.type !== 'OFTAdapter') // deploy only for OFTAdapter
    })
}
